...
bool ShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/shaders/shadow.vs", L"../Engine/shaders/shadow.ps");
	if(!result)
	{
		return false;
	}
...
bool ShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const DirectX::XMMATRIX& worldMatrix, const DirectX::XMMATRIX& viewMatrix, 
							   const DirectX::XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMapTexture, Light* light)
{
	bool result;
...
bool ShadowShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
											DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMapTexture, Light* light)
{
	HRESULT result;
	...
	dataPtr->lightView = XMMatrixTranspose(light->GetViewMatrix());
	dataPtr->lightProjection = XMMatrixTranspose(light->GetProjectionMatrix());
	...
	// Copy the lighting variables into the constant buffer.
	dataPtr3->lightDirection = light->GetDirection();
